...
#-------------------------------------------------------------------------------
            for j,k in zip([0,5,10],[5,10,len(self.Result.simulation)]):
            
                f.write('\\begin{table}\n \\centering\n \\caption{' + 
                           tableTitle + '}\n'+ '\\label{table1}\n' +
                       '\\begin{tabular}{\n')
                for i in range(len(self.Result.simulation[j:k])+1):    
                    f.write('c')
                f.write('}\n')
                f.write('\\hline\n'+ '\\textbf{Quantities}')
                for i in self.Result.simulation[j:k]:
                     f.write('& '+ '\\textbf{'+i+'}')
                f.write('\\\\ \n\\hline\n')

            
                f.write('$\\tau_{LET}$') 
                for i in self.Result.LET[j:k]:
                    f.write(' &  ')
                ...
                for i in self.Result.LET[j:k]:
                    f.write(' &  ')
                    f.write(' %9.7f' %float(i*30))
                    f.write('$s$')
                f.write('\\\\ \n')
 
                f.write('\\\\ \n\\hline\n')
                f.write('$\\Delta t$') 
                for i in self.Result.Dt[j:k]:
                    f.write(' &  ')
                    f.write(' %7.4E' %float(i))
                    f.write('$s$')
                f.write('\\\\ \n')
                
                f.write('$N\\Delta t (20LET)$') 
                for l,m in zip(self.Result.Dt[j:k], self.Result.LET[j:k]):
                    ndt = np.floor(m/float(l))*20

                    f.write(' &  ')
                    f.write(' %d' %  ndt )
                f.write('\\\\ \n')
                
                f.write('$N\\Delta t (30LET)$') 
                for l,m in zip(self.Result.Dt[j:k], self.Result.LET[j:k]):
                    ndt = np.floor(m/float(l))*30
                ...
                for i in self.Result.deborah[j:k]:
                     f.write(' &  ')
                     f.write(' %9.7f' %float(i))
                f.write('\\\\ \n')
 
                f.write('Reynolds$_\\lambda$ Number') 
                for i in self.Result.rey[j:k]:
                    f.write(' &  ')
                    f.write(' %9.7f' %float(i))
                f.write('\\\\ \n')
 
                f.write('Weissenberg Number') 
                for i in self.Result.We[j:k]:
                    f.write(' &  ')
                    f.write(' %9.7f' %float(i))
                f.write('\\\\ \n')
